...
private JButton makeHamburgerItem(String text, Color color, Runnable action) {
    ...
    btn.setBorder(BorderFactory.createEmptyBorder(9, 18, 9, 40));
...
private JSeparator makeMenuDivider() {
    JSeparator sep = new JSeparator();
    sep.setForeground(new Color(0xFFE0EE));
...
private void openShopDialog(Window owner) {
    JDialog dialog = (owner instanceof JFrame)
        ...
        : new JDialog((JDialog) owner, "🛍️ ร้านค้า", true);
    ...
    JLabel[] statusLabels = {
        makeStatusLbl("💰", String.valueOf(logic.getMoney()) + " บาท",  new Color(0x880E4F)),
    ...
    refreshUI(); // อัปเดตหลัง dialog ปิด
}
...
private void openWorkDialog(Window owner) {
    JDialog dialog = (owner instanceof JFrame)
        ...
        : new JDialog((JDialog) owner, "💼 ทำงาน", true);
...
private JPanel makeJobCard(int jobType, String emoji, String name, String desc,
    ...
    mid.add(nameLbl); mid.add(Box.createVerticalStrut(3)); mid.add(descLbl); mid.add(tags);
        ...
        @Override public void run() {
            ...
            for (int i = 0; i < q.choices.length; i++) {
                ...
                cb.addActionListener(ev -> {
                    ...
                    nextBtn.removeActionListener(nextBtn.getActionListeners().length > 0
                        ? nextBtn.getActionListeners()[0] : null);
